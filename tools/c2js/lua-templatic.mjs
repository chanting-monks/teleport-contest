...
const ALLOWED_NAMESPACES = new Set([
    ...
    // Lua stdlib modules used by the themerms-style files.  The
    // runtime provides JS-backed equivalents via the env object
    // passed to the generated module's default function.
    'math', 'table', 'string',
]);
...
const ALLOWED_HELPERS = new Set([
    ...
    'level_difficulty',
    'pline', 'impossible',
    // Lua stdlib globals exposed as direct helpers (no module prefix).
    // The runtime injects type() with Lua-style return values
    // ("table", "string", "number", "function", "nil", etc.) and
    // ipairs/pairs as iteration helpers (though renderer normally
    // inlines for-in loops without needing the iterator function).
    'type', 'ipairs', 'pairs',
    // Bitwise-op helpers for Lua metamethod dispatch.  Selection
    // userdata in NetHack overloads | (__bor), & (__band), and ~
    // (__bxor) for set arithmetic.  The renderer emits calls to
    // __lua_bor/band/bxor instead of raw JS infix; the env-provided
    // helpers dispatch on operand type (selection method call vs
    // integer bitwise).
    '__lua_bor', '__lua_band', '__lua_bxor',
]);
...
function parseLocal(p) {
    p.skipTrivia();
    const names = [p.parseIdent()];
    if (!names[0]) p.fail("expected name after 'local'");
    p.skipTrivia();
    while (p.peek() === ',') {
        p.advance();
        p.skipTrivia();
        const n = p.parseIdent();
        if (!n) p.fail("expected name after ',' in local decl");
        names.push(n);
        p.skipTrivia();
    }
    let values = null;
    ...
    // Preserve the single-name shape for back-compat (lots of
    // downstream code reads stmt.name / stmt.init directly).
    if (names.length === 1) {
        return { kind: 'local', name: names[0], init: values ? values[0] : null };
    }
    return { kind: 'multi_local', names, values };
}
...
const BIN_OPS = [
    // Lua 5.3+ precedence groups (lowest to highest); within each
    // group, left-associative.  Bitwise ops added between comparison
    // and `..` per Lua 5.4 reference manual §3.4.8.  NetHack's
    // selection userdata overloads `|` (union) and `&` (intersection)
    // via metamethods — the renderer emits these as JS infix ops,
    // and the selection bridge handles them at runtime via Symbol-
    // dispatched operators (or wrapper functions).
    { ops: ['or'], prec: 1 },
    { ops: ['and'], prec: 2 },
    { ops: ['<', '<=', '>', '>=', '==', '~='], prec: 3 },
    { ops: ['|'], prec: 4 },           // bitwise OR / set union
    { ops: ['~'], prec: 5 },           // bitwise XOR / set sym-diff
    { ops: ['&'], prec: 6 },           // bitwise AND / set intersection
    { ops: ['<<', '>>'], prec: 7 },    // bit shifts
    { ops: ['..'], prec: 8, right: true },
    { ops: ['+', '-'], prec: 9 },
    { ops: ['*', '/', '//', '%'], prec: 10 },
    // Unary at higher prec; handled in parseUnary
];

function lookupBinOp(p) {
    p.skipTrivia();
    const c = p.peek(), c2 = p.peek(1);
    // Two-char ops (must check before one-char)
    ...
    if (c === '.' && c2 === '.') return { op: '..', len: 2 };
    if (c === '/' && c2 === '/') return { op: '//', len: 2 };
    // One-char (including bitwise ops added for Lua 5.3+)
...
function parseValue(p) {
    ...
    // Vararg expression `...` — usable inside a vararg-function body
    // (as a call argument, table-constructor element, or assignment RHS).
    if (c === '.' && p.peek(1) === '.' && p.peek(2) === '.') {
...
// Parse one parameter slot.  Returns the param name (a plain
// identifier string) or the sentinel string '...' for the Lua
// vararg marker (which must be the last slot — caller's loop
// breaks after it).  Used by parseAnonFn for both anonymous and
// named function literals.
function parseParam(p) {
    p.skipTrivia();
    if (p.peek() === '.' && p.peek(1) === '.' && p.peek(2) === '.') {
        p.advance(); p.advance(); p.advance();
        return '...';
    }
    return p.parseIdent();
...
function parseAnonFn(p) {
    ...
    if (p.peek() !== ')') {
        ...
        while (true) {
            p.skipTrivia();
            if (p.peek() === ',' && params[params.length - 1] !== '...') {
                p.advance();
                params.push(parseParam(p));
            } else break;
        }
...
export function renderJsCalls(ast) {
    ...
    for (const s of body) {
        collectStmtLocals(ctx.scopeStack[0], s);
...
function collectEnvUses(stmts) {
    const used = new Set();
    // First, gather all names declared as `local` at any scope.  Files
    // like castle.lua / juiblex.lua have `local monster = { ... }`
    // where `monster` is a local array (not the env namespace).  If
    // we include `monster` in the destructure, the local-let
    // declaration collides with the function-parameter destructure
    // and JS errors with "Identifier 'monster' has already been
    // declared".  Exclude any name that's shadowed by a `local` from
    // the env requirement.
    //
    // Tradeoff: if a file has both `local NAME` (somewhere) AND uses
    // the env namespace `NAME` (elsewhere), refs to the namespace
    // outside the local's scope would land on the local.  This is
    // rare in practice — Lua scripts typically use either-or for a
    // given identifier.
    const locallyShadowed = new Set();
    function collectLocals(node) {
        if (!node || typeof node !== 'object') return;
        if (node.kind === 'local' && typeof node.name === 'string') {
            locallyShadowed.add(node.name);
        }
        if (node.kind === 'function_decl' && typeof node.name === 'string') {
            locallyShadowed.add(node.name);
        }
    ...
    stmts.forEach(collectLocals);

    function walk(node) {
        if (!node || typeof node !== 'object') return;
        if (node.kind === 'ident'
            && (ALLOWED_NAMESPACES.has(node.name) || ALLOWED_HELPERS.has(node.name))
            && !locallyShadowed.has(node.name)) {
            used.add(node.name);
        }
        // The renderer rewrites bitwise binops to __lua_bor/band/bxor
        // calls (for selection-metamethod dispatch); ensure those
        // helpers get destructured from env even though they never
        // appear as ident nodes in the AST.
        if (node.kind === 'binop') {
            if (node.op === '|') used.add('__lua_bor');
            else if (node.op === '&') used.add('__lua_band');
            else if (node.op === '~' && node.left) used.add('__lua_bxor');
        }
        for (const k of Object.keys(node)) {
            const v = node[k];
...
// Per-scope tracking of which local names have already had a JS
// `let` declaration emitted.  Lua allows redeclaring `local X` in
// the same block (each shadows the prior); JS `let` throws a SyntaxError
// for a duplicate identifier.  When emitting the second `local X = ...`
// at the same scope, drop the `let` keyword and emit a plain assignment.
// The Set used as the scope key is unique per scope-stack entry, so
// inner blocks (function bodies, if branches, for loops) get their
// own tracker automatically.
const SCOPE_EMITTED_LOCALS = new WeakMap();
function isLocalEmitted(scope, name) {
    const e = SCOPE_EMITTED_LOCALS.get(scope);
    return e ? e.has(name) : false;
}
function markLocalEmitted(scope, name) {
    let e = SCOPE_EMITTED_LOCALS.get(scope);
    if (!e) { e = new Set(); SCOPE_EMITTED_LOCALS.set(scope, e); }
    e.add(name);
}
...
function renderStmt(stmt, depth, ctx = null) {
    ...
    case 'local': {
        ...
        // If a prior `local NAME` at the same scope already emitted a
        // `let`, emit a plain assignment instead (JS forbids let-
        // redeclaration; Lua silently re-binds).
        const scope = ctx.scopeStack[ctx.scopeStack.length - 1];
        const isRedecl = isLocalEmitted(scope, stmt.name);
        markLocalEmitted(scope, stmt.name);
        scope.add(stmt.name);
        const jsn = jsName(stmt.name);
        if (stmt.init === null) {
            return isRedecl ? '' : `let ${jsn};`;
        }
        const initJs = renderExpr(stmt.init, depth, ctx);
        return isRedecl ? `${jsn} = ${initJs};` : `let ${jsn} = ${initJs};`;
    }
    case 'assign': {
    ...
    case 'multi_local': {
        // Lua `local a, b = e1, e2` → JS `let [a, b] = [e1, e2];`
        // (destructure shape gives uniform handling of mismatched
        // counts: shorter RHS → trailing undefined, longer RHS →
        // extras discarded, both matching Lua semantics).
        // Single-RHS multi-return case `local a, b = f()` is also
        // covered — emitted as `let [a, b] = f();`, which works if
        // `f()` returns an iterable (array).  Pure scalar-return
        // calls would throw at runtime; the dat/*.lua source paths
        // that use single-RHS multi-locals are dead code in the
        // contest runtime (e.g. nhcore.lua's mk_dgl_extrainfo).
        const scope = ctx.scopeStack[ctx.scopeStack.length - 1];
        for (const n of stmt.names) {
            markLocalEmitted(scope, n);
            scope.add(n);
        }
        const lhs = stmt.names.map(jsName).join(', ');
        if (stmt.values === null) {
            return `let ${lhs};`;
        }
        if (stmt.values.length === 1) {
            return `let [${lhs}] = ${renderExpr(stmt.values[0], depth, ctx)};`;
        }
        const rhs = stmt.values.map(v => renderExpr(v, depth, ctx)).join(', ');
        return `let [${lhs}] = [${rhs}];`;
    }
    case 'for_in': {
        ...
        if (iter.kind === 'call' && iter.callee?.kind === 'ident'
            ...
            for (const s of stmt.body) collectStmtLocals(bodyScope, s);
    ...
    case 'function_decl': {
        ...
        //
        // Async-ness is detected from the body: if any exprstmt
        // dispatches into ALLOWED_NAMESPACE (where renderer emits
        // \`await ns.method(...)\`), the function body has \`await\`
        // and must itself be \`async\`.  Otherwise, emit a plain
        // arrow — Lua-sync functions (like is_eligible, which only
        // reads room fields and calls type()/nh.X helpers) stay
        // sync in JS too, so callers' \`if (is_eligible(...))\`
        // evaluates the actual boolean rather than the truthy
        // Promise an async-by-default declaration would produce.
        const params = renderParams(stmt.fn.params);
        const bodyScope = new Set(stmt.fn.params.filter((p) => p !== '...'));
        for (const s of stmt.fn.body) collectStmtLocals(bodyScope, s);
        const innerCtx = { scopeStack: [...ctx.scopeStack, bodyScope] };
        const asyncKw = bodyNeedsAwait(stmt.fn.body) ? 'async ' : '';
        const lines = [`globalThis.${jsName(stmt.name)} = ${asyncKw}(${params}) => {`];
    ...
    case 'if': {
        ...
        for (let i = 0; i < stmt.branches.length; i++) {
            ...
            for (const s of br.body) collectStmtLocals(branchScope, s);
        ...
        if (stmt.else) {
            ...
            for (const s of stmt.else) collectStmtLocals(elseScope, s);
    ...
    case 'for_num': {
        ...
        // Lua semantics: lo, hi, and step are evaluated ONCE at loop
        // entry.  If hi is a side-effecting expression like `d(5,5)`
        // (PRNG-firing dice roll), evaluating it each iteration
        // (which is what JS's natural \`for(;cond;)\` would do) would
        // fire EXTRA PRNG calls.  Hoist hi/step into block-scoped
        // consts so they're evaluated once.  Wrap the for in a block
        // to keep the const declarations scoped.
        //
        // For the common case (no step or step >= 0) the condition is
        // i <= hi; for negative step it's i >= hi.  Emit generic form.
        const parts = [];
        parts.push(`{`);
        parts.push(`${indent(depth + 1)}const __hi = ${hi};`);
        parts.push(`${indent(depth + 1)}const __step = ${step};`);
        parts.push(`${indent(depth + 1)}for (let ${v} = ${lo}; __step > 0 ? ${v} <= __hi : ${v} >= __hi; ${v} += __step) {`);
        for (const s of stmt.body) parts.push(`${indent(depth + 2)}${renderStmt(s, depth + 2, innerCtx)}`);
...
// Detect whether a function body contains an exprstmt that the
// renderer would prefix with `await` (i.e., a dispatch into an
// ALLOWED_NAMESPACE).  Used to decide whether the enclosing
// function-decl / anonymous-function arrow needs the `async`
// keyword.  Lua-sync user-defined functions (is_eligible,
// lookup_by_name, etc.) don't await anything and should stay sync.
function bodyNeedsAwait(stmts) {
    if (!Array.isArray(stmts)) return false;
    for (const s of stmts) {
        if (!s) continue;
        if (s.kind === 'exprstmt' && isAsyncCall(s.expr)) return true;
        // Recurse into block-bearing statements.
        for (const k of Object.keys(s)) {
            const v = s[k];
            if (Array.isArray(v) && v.length && typeof v[0] === 'object') {
                if (bodyNeedsAwait(v)) return true;
            } else if (v && typeof v === 'object' && Array.isArray(v.body)) {
                if (bodyNeedsAwait(v.body)) return true;
            }
        }
        // if branches each carry { cond, body }; walk explicitly.
        if (s.kind === 'if' && Array.isArray(s.branches)) {
            for (const br of s.branches) if (bodyNeedsAwait(br.body)) return true;
            if (Array.isArray(s.else) && bodyNeedsAwait(s.else)) return true;
        }
    }
    return false;
}
...
function renderExpr(e, depth, ctx = null) {
    ...
    case 'index': {
        // Lua arrays are 1-indexed; JS arrays are 0-indexed.  Translate
        // by subtracting 1 from the subscript.
        //   - Numeric literal:  emit `[N - 1]` inline at compile time.
        //   - String literal:   leave unchanged (dict-like key lookup).
        //   - Variable / expr:  emit `[(<expr>) - 1]` — assumes numeric
        //     index.  String-keyed dynamic lookups `t[var]` where var
        //     holds a string break; the templatic subset doesn't
        //     normally use that pattern (idiomatic Lua uses dot notation
        //     `t.key` for known string keys), so this is acceptable.
        if (e.index.kind === 'str') {
            return `${renderExpr(e.target, depth, ctx)}[${renderExpr(e.index, depth, ctx)}]`;
        }
        if (e.index.kind === 'num') {
            return `${renderExpr(e.target, depth, ctx)}[${e.index.value - 1}]`;
        }
        return `${renderExpr(e.target, depth, ctx)}[(${renderExpr(e.index, depth, ctx)}) - 1]`;
    }
    case 'call': {
    ...
    case 'binop': {
        // Bitwise ops on NetHack selection userdata are metatable-
        // overloaded as set union (|), intersection (&), and
        // symmetric-difference (~).  JS \`|\`/\`&\`/\`~\` are integer
        // bitwise (coerce objects to 0), so we route through runtime
        // helpers that dispatch on operand type: if either side is a
        // Selection object (has the corresponding metamethod), call
        // it; else fall back to integer bitwise.  Helpers are
        // destructured from env at module top.
        if (e.op === '|') {
            return `__lua_bor(${renderExpr(e.left, depth, ctx)}, ${renderExpr(e.right, depth, ctx)})`;
        }
        if (e.op === '&') {
            return `__lua_band(${renderExpr(e.left, depth, ctx)}, ${renderExpr(e.right, depth, ctx)})`;
        }
        if (e.op === '~' && e.left) {
            // Binary ~: only here.  Unary ~ is handled in parseUnary.
            return `__lua_bxor(${renderExpr(e.left, depth, ctx)}, ${renderExpr(e.right, depth, ctx)})`;
        }
        // Lua \`//\` is integer-division (Lua 5.3+).  JS \`//\` is a
        // line-comment marker — emitting it as-is breaks the next
        // line of code.  Translate \`a // b\` → \`Math.floor(a / b)\`,
        // which matches Lua's semantics for positive operands; for
        // negative operands Lua floors and JS Math.floor matches.
        if (e.op === '//') {
            return `Math.floor(${renderExpr(e.left, depth, ctx)} / ${renderExpr(e.right, depth, ctx)})`;
        }
        // Lua \`==\` / \`~=\` map to JS loose \`==\` / \`!=\` rather than
        // strict.  Lua's nil-equality semantics treat missing table
        // fields as nil ("\`if t.missing_field == nil\`" → true); JS
        // returns \`undefined\` for missing object fields, and strict
        // \`undefined === null\` is false.  Loose \`==\` makes the two
        // interchangeable.  Cross-type comparisons (Lua \`1 == "1"\`
        // false vs JS \`1 == "1"\` true) are rare in practice and not
        // worth the false-negative-on-nil tradeoff.
        const opMap = { '~=': '!=', '==': '==', 'and': '&&', 'or': '||', '..': '+', '%': '%' };
    ...
    case 'function': {
        ...
        // Async-ness detected the same way as function_decl — only
        // mark async if the body has an ALLOWED_NAMESPACE dispatch.
        const params = renderParams(e.params);
        const innerCtx = { scopeStack: [...ctx.scopeStack, new Set(e.params.filter((p) => p !== '...'))] };
        const innerScope = innerCtx.scopeStack[innerCtx.scopeStack.length - 1];
        for (const s of e.body) collectStmtLocals(innerScope, s);
        const asyncKw = bodyNeedsAwait(e.body) ? 'async ' : '';
        const lines = ['(' + asyncKw + '(' + params + ') => {'];
        for (const s of e.body) lines.push(`${indent(depth + 2)}${renderStmt(s, depth + 2, innerCtx)}`);
        lines.push(`${indent(depth + 1)}})`);
        return lines.join('\n');
    }
    case 'vararg': {
        // Lua `...` reference inside a vararg function body.  Emits
        // a JS spread of the rest-parameter the renderer chose.  Only
        // legal in expression positions that already accept multi-
        // values — call args, table positional entries, return.
        return `...${VARARG_NAME}`;
    }
    case 'table': {
...
const JS_RESERVED = new Set([
    ...
    // Strict-mode reserved (ESM is always strict).  Caught by the
    // bundle-build path — used to be invisible because individual
    // data:URL imports failed silently per-file, but
    // `dat-bundle.js` is one module so a single bad ident kills
    // the whole runtime.  E.g. asmodeus.lua's `local protected`.
    'protected', 'public', 'private', 'interface', 'package',
    'static', 'implements',
]);
...
// Add any locals introduced by a single statement to the given
// scope Set.  Handles both single-name `local` and multi-name
// `multi_local`.  Used by every scope-building pre-scan loop so
// they stay in sync as new local-binding statement shapes land.
function collectStmtLocals(scope, s) {
    if (s.kind === 'local') scope.add(s.name);
    else if (s.kind === 'multi_local') for (const n of s.names) scope.add(n);
}

// Per-function name for the JS rest parameter that backs Lua's `...`
// vararg.  Picked to be unlikely to collide with translated source
// (no Lua source uses double-underscore + 'lua_' prefix as an ident).
// References to the Lua vararg expression `...` inside the body render
// as `...VARARG_NAME` (spread); table-constructor / table-pack uses
// would need additional handling not implemented in this pass.
const VARARG_NAME = '__lua_varargs';

// Render a param list, replacing the sentinel '...' (Lua vararg) with
// the JS rest-param form.  Vararg may only appear as the last slot
// (parser enforces this).
function renderParams(params) {
    return params.map((p) => (p === '...' ? `...${VARARG_NAME}` : jsName(p))).join(', ');
}
